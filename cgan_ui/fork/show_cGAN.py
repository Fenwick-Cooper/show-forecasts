...
def load_GAN_forecast(forecast_init_date, data_dir):
    ...
    file_name = (
        f"{data_dir}/east_africa-cgan_forecast-{d.year}{d.month:02}{d.day:02}.nc"
    )
...
def plot_GAN_forecast(
    ...
    if accumulation_time == "6h":
        ...
        if str(valid_time_start_hour) == "6":
            ...
            print(
                "ERROR: valid_time_start_hour must be 6, 12, 18 or 0 hours UTC or 'all'."
            )
            return
    ...
    for valid_time_idx in valid_time_idx_list:
        ...
        fig.suptitle(
            f"Jurre Brishti cGAN forecast: Valid {valid_time} - {valid_time + valid_time_delta} UTC"
        )  # Overall title
...
def plot_GAN_ensemble(
    ...
    fig.suptitle(
        f"Jurre Brishti cGAN ensemble: Valid {valid_time} - {valid_time + timedelta(hours=6)} UTC"
    )  # Overall title
...
def plot_GAN_local_histograms(
    data, location_name, country=None, latitude=None, longitude=None, plot_units="mm/h"
):
